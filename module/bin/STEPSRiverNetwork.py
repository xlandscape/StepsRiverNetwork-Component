...
class STEPSRiverNetwork:
    ...
    def __init__(self,fpath:str,
                 ...
                 db_pars:list= ["MASS_SW","MASS_SED","MASS_SED_DEEP","PEC_SW",
                                "PEC_SED","DEGR_SW","DEGR_SED"],
                 db_funcs:list=[np.take,np.take,np.take,np.mean,np.mean,np.sum,np.sum],
                 db_kwargs:list=[{'indices':-1,"axis":0},{'indices':-1,"axis":0},
        ...
        self.reachlist = self.reachlist.sort_values("key")
        ...
        if self.filetype == "h5":
            ...
            #TODO: test
            self.simulated.sort_index(level=0,inplace=True)
            
            self.n_time_input = len(self.simulated.index.levels[0])

            # reshape hydrological input data
            self.flow = self.simulated.flow.values.reshape([self.n_time_input,
                                                            self.n_reaches])
            self.vol = self.simulated.volume.values.reshape([self.n_time_input,
                                                             self.n_reaches])
            self.area = self.simulated.area.values.reshape([self.n_time_input,
                                                            self.n_reaches])
        ...
        self.input_sw = np.zeros([self.n_time_input,self.n_reaches],
                                 dtype=np.float32)
    
        # spray drift
        self.spraydrift = pd.read_csv(os.path.join(self.fpath,
                                                   "SprayDriftList.csv"))
        ...
        # dave input_sw
        fh5.create_dataset("input_sw", (self.n_time_input,self.n_reaches), 
                             dtype="f4",compression="gzip",
                             compression_opts=4)
        fh5["input_sw"][:] = self.input_sw
        fh5.close()        
    ...
    def __call__(self,printres:bool=False):
        ...
        for tinput in range(self.t0_index,self.tn_index,1):
            ...
            flow_at_t = self.flow[tinput]
            vol_at_t = self.vol[tinput]
            tmp_at_t = self.temp[tinput]
            ...
            if (tinternal % 1440 == 0):   print(self.time[tinternal])
            ...
            for step in range(dt_sub):
                ...
                if (cond_inp or cond_sw or cond_sed):
                    ...
                    self.printout(tinput," input ",input_sw_at_t,printres)
                    ...
                    #TODO: test
                    self.load_outflow = np.multiply(self.efate.PEC_SW,flow_at_t,out=np.zeros(flow_at_t.shape),where=self.efate.PEC_SW>0.000001)
                    
                    #self.load_outflow = self.efate.PEC_SW * flow_at_t
                    ...
                    if zero_state == False:
                        ...
                        for var in ["MASS_SW","MASS_SED","MASS_SED_DEEP"]:
...
#                    ds.flush()
    ...
    def save(self,texternal,step):
        ...
        if step == 59: 
            ...
            for par,func,kwargs,ds in zip(self.db_pars,self.db_funcs,self.db_kwargs,self.db):
                dset = self.fh5[par]
                dset[texternal] = func(ds,**kwargs) #ds.max(axis=0)
