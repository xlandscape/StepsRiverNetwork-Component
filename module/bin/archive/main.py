...
def set_flows():
    dat = pd.read_csv(r"c:\00_copy\_STEPSRivernetwork_v092\projects\h1\__HydroList.csv")
    dat.set_index("key",inplace=True)
    dat.volume=10
    dat.flow=10
    dat.area=10
    dat.to_csv(r"c:\00_copy\_STEPSRivernetwork_v092\projects\h1\HydroList.csv")
...
def process_output(run,runtime,TWA=168,func_TWA=np.nanmax):
    """
    """
    ...
    reachlist = pd.read_csv(os.path.join(fpath,"ReachList.csv"),
                            index_col="key")
    reachnames = list(reachlist.index)
    ...
    s+="# Number of reaches: " + str(len(reachlist))+"\n"
...
#    FLAGS.folder = "c:/00_copy/_STEPSRivernetwork_v093/projects/"
#    FLAGS.runlist = "h2"
